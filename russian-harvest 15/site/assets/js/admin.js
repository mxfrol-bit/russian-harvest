...
(function () {
  ...
  function init() {
    ...
    // Работает для всех динамически созданных модалок:
    //  - клик на любой .modal-close → закрывает родительский wrapper
    //  - клик на любой .modal-backdrop → закрывает родительский wrapper
    //  - Esc → закрывает САМУЮ ПОСЛЕДНЮЮ открытую модалку (стек)
    document.addEventListener('click', e => {
      // Close button anywhere — find nearest wrap and remove it
      ...
      // Backdrop click — only if clicked directly on backdrop itself
      if (e.target.classList.contains('modal-backdrop') && e.target.classList.contains('on')) {
        const wrap = findModalWrap(e.target);
        if (wrap) closeModalWrap(wrap);
      }
    }, true); // capture phase to handle before inner handlers

    document.addEventListener('keydown', e => {
      if (e.key === 'Escape') {
        // Close topmost modal (last child in body)
        const allWraps = Array.from(document.querySelectorAll('div')).filter(d =>
          d.parentNode === document.body &&
          d.querySelector('.modal-backdrop.on') &&
          d.querySelector('.modal.on')
        );
        if (allWraps.length) {
          closeModalWrap(allWraps[allWraps.length - 1]);
        }
      }
    });
  }

  // Find the wrapper div that contains a modal element.
  // Modals created by admin.js are wrapped in a single <div> appended to body.
  function findModalWrap(el) {
    let cur = el;
    while (cur && cur.parentNode !== document.body) cur = cur.parentNode;
    if (!cur) return null;
    // Make sure this wrap actually contains a modal
    if (cur.querySelector && (cur.querySelector('.modal-backdrop') || cur.querySelector('.modal'))) {
      return cur;
    }
    return null;
  }
