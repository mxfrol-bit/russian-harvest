...
(function () {
  ...
  api.import1c = {
     ...
     * Безопасно: ключ Railway-сервиса хранится в Supabase secrets,
     * не виден в браузере. Edge Function проверяет JWT админа двойной
     * валидацией (auth.uid + profiles.role='admin'), затем форвардит
     * запрос на /analyze/{filename} Railway-сервиса.
     *
     * @param {string} filename
     */
    async aiAnalyze(filename) {
      ...
      // Edge Function endpoint: <SUPABASE_URL>/functions/v1/import-1c-analyze
      ...
      if (!resp.ok) {
        ...
        // Если функция ещё не задеплоена, упадёт с 404 — даём ясную подсказку
        if (resp.status === 404) {
